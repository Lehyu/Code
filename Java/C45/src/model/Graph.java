...
public class Graph {
    ...
    public void delVec(Integer v){
        ...
        while(it.hasNext()){
            ...
            if(v.equals(bag.getLabel())){
                Iterator<Integer> bag_it = bag.getBag().iterator();
                while(bag_it.hasNext()){
                	Integer w = bag_it.next();
                	//delete node v from w's set
                	delEdgeOfNode(w, v);
                }
                it.remove();
                break;
    ...
    * */
    ...
    private void delEdgeOfNode(Integer w, Integer v){
    	//System.out.print("In delEdg "+i+" "+j);
        for(int index = 0; index < adj.size(); index++){
        	if(w.equals(adj.get(index).getLabel())){
        		adj.get(index).remove(v);
        		break;
        	}
        }
    }
